...
async function verify() {
  ...
  try {
    ...
    await pool.query('UPDATE users SET role = $1 WHERE username = $2', ['admin', adminUser]);
    await pool.query('UPDATE users SET role = $1 WHERE username = $2', ['petugas gizi', nutriUser]);
    ...
    if (menuId) await pool.query('DELETE FROM menus WHERE id = $1', [menuId]);
    await pool.query('DELETE FROM users WHERE username IN ($1, $2)', [adminUser, nutriUser]);
